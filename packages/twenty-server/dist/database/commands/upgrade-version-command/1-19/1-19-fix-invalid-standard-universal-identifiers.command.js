...
Object.defineProperty(exports, "FixInvalidStandardUniversalIdentifiersCommand", {
    enumerable: true,
    get: function() {
        return FixInvalidStandardUniversalIdentifiersCommand;
    }
});
...
const _standardagentconstant = require("../../../../engine/workspace-manager/twenty-standard-application/constants/standard-agent.constant");
...
const _standardskillconstant = require("../../../../engine/workspace-manager/twenty-standard-application/constants/standard-skill.constant");
...
const OLD_ROLE_ADMIN_UNIVERSAL_IDENTIFIER = '20202020-0001-0001-0001-000000000001';
const OLD_AGENT_HELPER_UNIVERSAL_IDENTIFIER = '20202020-0002-0001-0001-000000000004';
const STANDARD_OBJECT_MISMATCHES = [
    ...
    },
    {
        ...
        newValue: _standardskillconstant.STANDARD_SKILL['workflow-building'].universalIdentifier
    },
    {
        table: 'core."skill"',
        column: '"universalIdentifier"',
        oldValue: '20202020-e225-f5c7-3d56-45feaa36f2e6',
        newValue: _standardskillconstant.STANDARD_SKILL['data-manipulation'].universalIdentifier
    },
    {
        table: 'core."skill"',
        column: '"universalIdentifier"',
        oldValue: '20202020-398f-0d7a-82db-4f43bc7e7044',
        newValue: _standardskillconstant.STANDARD_SKILL['dashboard-building'].universalIdentifier
    },
    {
        table: 'core."skill"',
        column: '"universalIdentifier"',
        oldValue: '20202020-c66a-5fed-4a74-46e0b42a6332',
        newValue: _standardskillconstant.STANDARD_SKILL['metadata-building'].universalIdentifier
    },
    {
        table: 'core."skill"',
        column: '"universalIdentifier"',
        oldValue: '20202020-db75-4fca-6813-4c7db0f964a0',
        newValue: _standardskillconstant.STANDARD_SKILL.research.universalIdentifier
    },
    {
        table: 'core."skill"',
        column: '"universalIdentifier"',
        oldValue: '20202020-5eb9-e775-cf4e-4f22be7be362',
        newValue: _standardskillconstant.STANDARD_SKILL['code-interpreter'].universalIdentifier
    },
    {
        table: 'core."skill"',
        column: '"universalIdentifier"',
        oldValue: '20202020-2c7f-5b77-dfa4-494b84752ab7',
        newValue: _standardskillconstant.STANDARD_SKILL.xlsx.universalIdentifier
    },
    {
        table: 'core."skill"',
        column: '"universalIdentifier"',
        oldValue: '20202020-c3d1-e0c9-2f93-45648b8bbd26',
        newValue: _standardskillconstant.STANDARD_SKILL.pdf.universalIdentifier
    },
    {
        table: 'core."skill"',
        column: '"universalIdentifier"',
        oldValue: '20202020-6f15-2432-0537-4e23a2efd1cb',
        newValue: _standardskillconstant.STANDARD_SKILL.docx.universalIdentifier
    },
    {
        table: 'core."skill"',
        column: '"universalIdentifier"',
        oldValue: '20202020-c81b-baf8-5255-4c34bd0eac9b',
        newValue: _standardskillconstant.STANDARD_SKILL.pptx.universalIdentifier
    }
];
let FixInvalidStandardUniversalIdentifiersCommand = class FixInvalidStandardUniversalIdentifiersCommand extends _activeorsuspendedworkspacesmigrationcommandrunner.ActiveOrSuspendedWorkspacesMigrationCommandRunner {
    async runOnWorkspace({ workspaceId, options }) {
        const isDryRun = options?.dryRun ?? false;
        this.logger.log(`${isDryRun ? '[DRY RUN] ' : ''}Fixing universal identifiers for workspace ${workspaceId}`);
        if (isDryRun) {
            this.logger.log(`[DRY RUN] Would fix role, agent, skill, and ${STANDARD_OBJECT_MISMATCHES.length} standard object universal identifier mismatches in workspace ${workspaceId}. Skipping.`);
            return;
        }
        ...
        try {
            let totalUpdated = 0;
            const roleResult = await queryRunner.query(`UPDATE core."role"
         SET "universalIdentifier" = $1
         WHERE "workspaceId" = $2
           AND "universalIdentifier" = $3`, [
                _standardroleconstant.STANDARD_ROLE.admin.universalIdentifier,
                workspaceId,
                OLD_ROLE_ADMIN_UNIVERSAL_IDENTIFIER
            ]);
            const roleUpdatedCount = roleResult?.[1] ?? 0;
            if (roleUpdatedCount > 0) {
                this.logger.log(`Updated ${roleUpdatedCount} role universalIdentifier in workspace ${workspaceId}`);
                totalUpdated += roleUpdatedCount;
            }
            const agentResult = await queryRunner.query(`UPDATE core."agent"
         SET "universalIdentifier" = $1
         WHERE "workspaceId" = $2
           AND "universalIdentifier" = $3`, [
                _standardagentconstant.STANDARD_AGENT.helper.universalIdentifier,
                workspaceId,
                OLD_AGENT_HELPER_UNIVERSAL_IDENTIFIER
            ]);
            const agentUpdatedCount = agentResult?.[1] ?? 0;
            if (agentUpdatedCount > 0) {
                this.logger.log(`Updated ${agentUpdatedCount} agent universalIdentifier in workspace ${workspaceId}`);
                totalUpdated += agentUpdatedCount;
            }
            const updateCountsByTable = {};
            for (const mismatch of STANDARD_OBJECT_MISMATCHES){
                const result = await queryRunner.query(`UPDATE ${mismatch.table}
           ...
           WHERE "workspaceId" = $2
             AND ${mismatch.column} = $3`, [
                ...
                if (updatedCount > 0) {
                    ...
                    updateCountsByTable[tableKey] = (updateCountsByTable[tableKey] ?? 0) + updatedCount;
                    totalUpdated += updatedCount;
                }
            }
            for (const [tableKey, count] of Object.entries(updateCountsByTable)){
                this.logger.log(`Updated ${count} rows in ${tableKey} for workspace ${workspaceId}`);
            }
            if (totalUpdated === 0) {
                this.logger.log(`No universal identifiers needed updating in workspace ${workspaceId}`);
            } else {
                this.logger.log(`Updated ${totalUpdated} total universal identifiers in workspace ${workspaceId}`);
...
FixInvalidStandardUniversalIdentifiersCommand = _ts_decorate([
    (0, _nestcommander.Command)({
        name: 'upgrade:1-19:fix-invalid-standard-universal-identifiers',
        description: 'Fix invalid universalIdentifier values in core tables to comply with UUID v4 format'
    }),
...
//# sourceMappingURL=1-19-fix-invalid-standard-universal-identifiers.command.js.map