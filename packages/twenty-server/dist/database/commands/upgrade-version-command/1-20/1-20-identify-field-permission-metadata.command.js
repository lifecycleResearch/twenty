...
Object.defineProperty(exports, "IdentifyFieldPermissionMetadataCommand", {
    enumerable: true,
    get: function() {
        return IdentifyFieldPermissionMetadataCommand;
    }
});
...
let IdentifyFieldPermissionMetadataCommand = class IdentifyFieldPermissionMetadataCommand extends _activeorsuspendedworkspacesmigrationcommandrunner.ActiveOrSuspendedWorkspacesMigrationCommandRunner {
    async runOnWorkspace({ options }) {
        if (this.hasRunOnce) {
            this.logger.log('Skipping has already been run once IdentifyFieldPermissionMetadataCommand');
            return;
        }
        if (options.dryRun) {
            return;
        ...
        try {
            const repository = queryRunner.manager.getRepository(_fieldpermissionentity.FieldPermissionEntity);
            ...
            const toUpdate = withNullApplicationId.filter((fieldPermission)=>(0, _utils.isDefined)(fieldPermission.role?.applicationId));
            const toRemove = withNullApplicationId.filter((fieldPermission)=>!(0, _utils.isDefined)(fieldPermission.role?.applicationId));
            for (const fieldPermission of toUpdate){
                fieldPermission.applicationId = fieldPermission.role.applicationId;
                fieldPermission.universalIdentifier = fieldPermission.universalIdentifier ?? (0, _uuid.v4)();
            }
            ...
            for (const fieldPermission of withNullUniversalIdentifier){
                fieldPermission.universalIdentifier = (0, _uuid.v4)();
            }
            if (withNullUniversalIdentifier.length > 0) {
                await repository.save(withNullUniversalIdentifier);
            }
            await queryRunner.commitTransaction();
            this.logger.log('Successfully run IdentifyFieldPermissionMetadataCommand');
            this.hasRunOnce = true;
        } catch (error) {
            await queryRunner.rollbackTransaction();
            this.logger.error(`Rolling back IdentifyFieldPermissionMetadataCommand: ${error.message}`);
            throw error;
        } finally{
...
IdentifyFieldPermissionMetadataCommand = _ts_decorate([
    (0, _nestcommander.Command)({
        name: 'upgrade:1-20:identify-field-permission-metadata',
        description: 'Identify field permission metadata (backfill universalIdentifier and applicationId)'
    }),
...
//# sourceMappingURL=1-20-identify-field-permission-metadata.command.js.map