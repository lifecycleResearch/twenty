...
_export(exports, {
    get MESSAGE_DATA_SEEDS () {
        return MESSAGE_DATA_SEEDS;
    },
    get MESSAGE_DATA_SEED_COLUMNS () {
        return MESSAGE_DATA_SEED_COLUMNS;
    },
    get MESSAGE_DATA_SEED_IDS () {
        return MESSAGE_DATA_SEED_IDS;
    }
});
const _messagethreaddataseedsconstant = require("./message-thread-data-seeds.constant");
const MESSAGE_DATA_SEED_COLUMNS = [
    'id',
    'createdAt',
    'updatedAt',
    'deletedAt',
...
const MESSAGE_DATA_SEED_IDS = GENERATE_MESSAGE_IDS();
...
const GENERATE_MESSAGE_SEEDS = ()=>{
    const MESSAGE_SEEDS = [];
    const THREAD_IDS = Object.keys(_messagethreaddataseedsconstant.MESSAGE_THREAD_DATA_SEED_IDS).map((key)=>_messagethreaddataseedsconstant.MESSAGE_THREAD_DATA_SEED_IDS[key]);
    for(let INDEX = 1; INDEX <= 600; INDEX++){
        ...
        const MESSAGE_DATE = new Date(NOW.getTime() - RANDOM_DAYS_OFFSET * 24 * 60 * 60 * 1000);
        MESSAGE_DATE.setHours(8 + Math.floor(Math.random() * 10), Math.floor(Math.random() * 60), 0, 0);
        MESSAGE_SEEDS.push({
...
const MESSAGE_DATA_SEEDS = GENERATE_MESSAGE_SEEDS();

//# sourceMappingURL=message-data-seeds.constant.js.map