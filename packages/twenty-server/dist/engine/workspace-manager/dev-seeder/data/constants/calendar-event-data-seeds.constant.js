...
_export(exports, {
    get CALENDAR_EVENT_DATA_SEEDS () {
        return CALENDAR_EVENT_DATA_SEEDS;
    },
    get CALENDAR_EVENT_DATA_SEED_COLUMNS () {
        return CALENDAR_EVENT_DATA_SEED_COLUMNS;
    },
    get CALENDAR_EVENT_DATA_SEED_IDS () {
        return CALENDAR_EVENT_DATA_SEED_IDS;
    }
});
const CALENDAR_EVENT_DATA_SEED_COLUMNS = [
    'id',
    'title',
    'isCanceled',
    'isFullDay',
...
const GENERATE_CALENDAR_EVENT_IDS = ()=>{
    const CALENDAR_EVENT_IDS = {};
    for(let INDEX = 1; INDEX <= 800; INDEX++){
        const HEX_INDEX = INDEX.toString(16).padStart(4, '0');
        CALENDAR_EVENT_IDS[`ID_${INDEX}`] = `20202020-${HEX_INDEX}-4e7c-8001-123456789cde`;
    }
    return CALENDAR_EVENT_IDS;
};
const CALENDAR_EVENT_DATA_SEED_IDS = GENERATE_CALENDAR_EVENT_IDS();
...
const GENERATE_CALENDAR_EVENT_SEEDS = ()=>{
    const CALENDAR_EVENT_SEEDS = [];
    for(let INDEX = 1; INDEX <= 800; INDEX++){
        ...
        const EVENT_DATE = new Date(NOW.getTime() + RANDOM_DAYS_OFFSET * 24 * 60 * 60 * 1000);
        ...
        const CONFERENCE_SOLUTION = TEMPLATE.conferenceSolutions[Math.floor(Math.random() * TEMPLATE.conferenceSolutions.length)];
        // 5% chance of being cancelled
        const IS_CANCELLED = Math.random() < 0.05;
        // Generate conference link if it's an online meeting
        const CONFERENCE_LINK = [
            'Zoom',
            'Teams',
            'Google Meet'
        ].includes(CONFERENCE_SOLUTION) ? `https://${CONFERENCE_SOLUTION.toLowerCase().replace(' ', '')}.com/j/${Math.floor(Math.random() * 9000000000) + 1000000000}` : '';
        CALENDAR_EVENT_SEEDS.push({
            ...
            externalCreatedAt: new Date(START_TIME.getTime() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString(),
            externalUpdatedAt: new Date(START_TIME.getTime() - Math.random() * 24 * 60 * 60 * 1000).toISOString(),
...
const CALENDAR_EVENT_DATA_SEEDS = GENERATE_CALENDAR_EVENT_SEEDS();

//# sourceMappingURL=calendar-event-data-seeds.constant.js.map