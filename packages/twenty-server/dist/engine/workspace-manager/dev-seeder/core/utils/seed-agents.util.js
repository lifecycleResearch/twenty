...
_export(exports, {
    get AGENT_CHAT_MESSAGE_DATA_SEED_IDS () {
        return AGENT_CHAT_MESSAGE_DATA_SEED_IDS;
    },
    get AGENT_CHAT_MESSAGE_PART_DATA_SEED_IDS () {
        return AGENT_CHAT_MESSAGE_PART_DATA_SEED_IDS;
    },
    get AGENT_CHAT_THREAD_DATA_SEED_IDS () {
        return AGENT_CHAT_THREAD_DATA_SEED_IDS;
    },
    get AGENT_DATA_SEED_IDS () {
        return AGENT_DATA_SEED_IDS;
    },
    get seedAgents () {
        return seedAgents;
    }
...
const AGENT_CHAT_MESSAGE_PART_DATA_SEED_IDS = {
...
const seedChatThreads = async ({ queryRunner, schemaName, workspaceId })=>{
    let threadId;
    let userWorkspaceId;
    if (workspaceId === _seederworkspacesconstant.SEED_APPLE_WORKSPACE_ID) {
        ...
        userWorkspaceId = _seeduserworkspacesutil.USER_WORKSPACE_DATA_SEED_IDS.TIM_ACME;
    } else {
        throw new Error(`Unsupported workspace ID for agent chat thread seeding: ${workspaceId}`);
    }
    const now = new Date();
    await queryRunner.manager.createQueryBuilder().insert().into(`${schemaName}.${agentChatThreadTableName}`, [
        'id',
        'userWorkspaceId',
        'createdAt',
        'updatedAt'
...
const seedChatMessages = async ({ queryRunner, schemaName, workspaceId, threadId })=>{
    let messageIds;
    let partIds;
    let turnIds;
    let messages;
        ...
        messageParts = [
            {
                ...
                orderIndex: 0,
                type: 'text',
                textContent: 'Hello! Can you help me understand our current product roadmap and key metrics?',
                createdAt: new Date(baseTime.getTime())
            },
            {
                id: partIds[1],
                ...
                orderIndex: 0,
                type: 'text',
                textContent: "Hello! I'd be happy to help you understand Apple's product roadmap and metrics. Based on your workspace data, I can see you have various projects and initiatives tracked. What specific aspect would you like to explore - product development timelines, user engagement metrics, or revenue targets?",
                createdAt: new Date(baseTime.getTime() + 5 * 60 * 1000)
            },
            {
                id: partIds[2],
                ...
                orderIndex: 0,
                type: 'text',
                textContent: "I'd like to focus on our user engagement metrics and how they're trending over the last quarter.",
                createdAt: new Date(baseTime.getTime() + 10 * 60 * 1000)
            },
            {
                id: partIds[3],
        ...
        messageParts = [
            {
                ...
                orderIndex: 0,
                type: 'text',
                textContent: 'What are the current startup trends and which companies in our portfolio are performing best?',
                createdAt: new Date(baseTime.getTime())
            },
            {
                id: partIds[1],
                ...
                orderIndex: 0,
                type: 'text',
                textContent: 'Hello! I can help you analyze startup trends and portfolio performance. From your YCombinator workspace data, I can see strong performance in AI/ML startups, particularly in the B2B SaaS space. Several companies are showing 40%+ month-over-month growth. Would you like me to provide specific company performance metrics or focus on broader industry trends?',
                createdAt: new Date(baseTime.getTime() + 3 * 60 * 1000)
            },
            {
                id: partIds[2],
                ...
                orderIndex: 0,
                type: 'text',
                textContent: 'Please focus on our top 5 performing companies and their key metrics.',
                createdAt: new Date(baseTime.getTime() + 8 * 60 * 1000)
            },
            {
                id: partIds[3],
    ...
    await queryRunner.manager.createQueryBuilder().insert().into(`${schemaName}.${agentTurnTableName}`, [
        'id',
        'threadId',
        'createdAt'
    ]).orIgnore().values(turns).execute();
    await queryRunner.manager.createQueryBuilder().insert().into(`${schemaName}.${agentMessageTableName}`, [
        'id',
        'threadId',
        'turnId',
        'role',
        'createdAt'
    ]).orIgnore().values(messages).execute();
    await queryRunner.manager.createQueryBuilder().insert().into(`${schemaName}.${agentMessagePartTableName}`, [
        'id',
        'messageId',
        'orderIndex',
        'type',
        'textContent',
        'createdAt'
    ]).orIgnore().values(messageParts).execute();
};
const seedAgents = async ({ queryRunner, schemaName, workspaceId })=>{
    if (workspaceId === _seederworkspacesconstant.SEED_APPLE_WORKSPACE_ID) {
        return;
    }
...
//# sourceMappingURL=seed-agents.util.js.map