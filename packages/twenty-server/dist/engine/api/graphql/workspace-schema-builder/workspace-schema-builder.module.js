"use strict";
Object.defineProperty(exports, "__esModule", {
    value: true
});
Object.defineProperty(exports, "WorkspaceSchemaBuilderModule", {
    enumerable: true,
    get: function() {
        return WorkspaceSchemaBuilderModule;
    }
});
const _common = require("@nestjs/common");
const _workspaceresolverbuildermodule = require("../workspace-resolver-builder/workspace-resolver-builder.module");
const _gqltypegenerator = require("./graphql-type-generators/gql-type.generator");
const _typemapperservice = require("./services/type-mapper.service");
const _workspacegraphqlschemafactory = require("./workspace-graphql-schema.factory");
function _ts_decorate(decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for(var i = decorators.length - 1; i >= 0; i--)if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
}
let WorkspaceSchemaBuilderModule = class WorkspaceSchemaBuilderModule {
};
WorkspaceSchemaBuilderModule = _ts_decorate([
    (0, _common.Module)({
        imports: [
            _workspaceresolverbuildermodule.WorkspaceResolverBuilderModule
        ],
        providers: [
            _typemapperservice.TypeMapperService,
            _gqltypegenerator.GqlTypeGenerator,
            _workspacegraphqlschemafactory.WorkspaceGraphQLSchemaGenerator
        ],
        exports: [
            _workspacegraphqlschemafactory.WorkspaceGraphQLSchemaGenerator
        ]
    })
], WorkspaceSchemaBuilderModule);

//# sourceMappingURL=workspace-schema-builder.module.js.map