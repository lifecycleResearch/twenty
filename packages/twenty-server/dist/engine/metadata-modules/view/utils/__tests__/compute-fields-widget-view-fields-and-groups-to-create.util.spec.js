...
const _computefieldswidgetviewfieldsandgroupstocreateutil = require("../compute-fields-widget-view-fields-and-groups-to-create.util");
...
describe('computeFieldsWidgetViewFieldsAndGroupsToCreate', ()=>{
    it('should create a "General" group with standard fields', ()=>{
        ...
        const result = (0, _computefieldswidgetviewfieldsandgroupstocreateutil.computeFieldsWidgetViewFieldsAndGroupsToCreate)({
        ...
        result.flatViewFieldsToCreate.forEach((vf)=>{
            expect(vf.viewFieldGroupUniversalIdentifier).toBe(result.flatViewFieldGroupsToCreate[0].universalIdentifier);
            expect(vf.isVisible).toBe(true);
            expect(vf.size).toBe(_defaultviewfieldsizeconstant.DEFAULT_VIEW_FIELD_SIZE);
    ...
    it('should create an "Other" group when custom fields exist', ()=>{
        ...
        const generalGroupUid = result.flatViewFieldGroupsToCreate[0].universalIdentifier;
        const otherGroupUid = result.flatViewFieldGroupsToCreate[1].universalIdentifier;
        const generalFields = result.flatViewFieldsToCreate.filter((vf)=>vf.viewFieldGroupUniversalIdentifier === generalGroupUid);
        const otherFields = result.flatViewFieldsToCreate.filter((vf)=>vf.viewFieldGroupUniversalIdentifier === otherGroupUid);
    ...
    it('should exclude deletedAt field', ()=>{
        ...
        expect(result.flatViewFieldsToCreate[0].fieldMetadataUniversalIdentifier).toBe(fields[0].universalIdentifier);
    });
    ...
    it('should exclude id field unless it is the label identifier', ()=>{
        ...
        const resultWithout = (0, _computefieldswidgetviewfieldsandgroupstocreateutil.computeFieldsWidgetViewFieldsAndGroupsToCreate)({
            objectFlatFieldMetadatas: fields,
            viewUniversalIdentifier,
            flatApplication,
            labelIdentifierFieldMetadataUniversalIdentifier: null
        ...
        const resultWith = (0, _computefieldswidgetviewfieldsandgroupstocreateutil.computeFieldsWidgetViewFieldsAndGroupsToCreate)({
            objectFlatFieldMetadatas: fields,
            viewUniversalIdentifier,
            flatApplication,
    ...
    it('should place label identifier field at position 0', ()=>{
        ...
        const labelField = result.flatViewFieldsToCreate.find((vf)=>vf.fieldMetadataUniversalIdentifier === labelUid);
        expect(labelField).toBeDefined();
        expect(labelField.position).toBe(0);
        // Other fields should have higher positions
        const otherPositions = result.flatViewFieldsToCreate.filter((vf)=>vf.fieldMetadataUniversalIdentifier !== labelUid).map((vf)=>vf.position);
    ...
    it('should make RELATION and MORPH_RELATION fields invisible by default', ()=>{
        ...
        const viewFieldForName = result.flatViewFieldsToCreate.find((vf)=>vf.fieldMetadataUniversalIdentifier === fields[0].universalIdentifier);
        const viewFieldForCompany = result.flatViewFieldsToCreate.find((vf)=>vf.fieldMetadataUniversalIdentifier === fields[1].universalIdentifier);
        const viewFieldForTarget = result.flatViewFieldsToCreate.find((vf)=>vf.fieldMetadataUniversalIdentifier === fields[2].universalIdentifier);
        const viewFieldForCustomRelation = result.flatViewFieldsToCreate.find((vf)=>vf.fieldMetadataUniversalIdentifier === fields[3].universalIdentifier);
...
//# sourceMappingURL=compute-fields-widget-view-fields-and-groups-to-create.util.spec.js.map