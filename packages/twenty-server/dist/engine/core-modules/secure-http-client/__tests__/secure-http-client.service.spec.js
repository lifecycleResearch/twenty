...
const _securehttpclientservice = require("../secure-http-client.service");
...
describe('SecureHttpClientService', ()=>{
    describe('getHttpClient', ()=>{
        it('should return a plain axios instance when safe mode is off', ()=>{
            const service = new _securehttpclientservice.SecureHttpClientService(createMockConfigService());
        ...
        it('should return an axios instance with SSRF-safe agents when safe mode is on', ()=>{
            ...
            const client = service.getHttpClient();
            expect(client).toBeDefined();
            expect(client.defaults.httpAgent).toBeInstanceOf(_http.Agent);
            expect(client.defaults.httpsAgent).toBeInstanceOf(_https.Agent);
        });
        ...
        it('should configure axios-retry when retries is greater than 0', ()=>{
            jest.mocked(_axiosretry.default).mockClear();
            const service = new _securehttpclientservice.SecureHttpClientService(createMockConfigService());
            const client = service.getHttpClient({
                retries: 2,
                shouldResetTimeout: true
            });
            expect(_axiosretry.default).toHaveBeenCalledWith(client, {
                retries: 2,
                shouldResetTimeout: true,
        ...
        it('should not configure axios-retry when retries is 0', ()=>{
            ...
            expect(_axiosretry.default).not.toHaveBeenCalled();
        });
    ...
    describe('protocol validation interceptor', ()=>{
        it('should allow http URLs when safe mode is on', ()=>{
            ...
            const client = service.getHttpClient();
            const interceptorHandlers = client.interceptors.request.handlers;
            const protocolInterceptor = interceptorHandlers[0].fulfilled;
            expect(()=>protocolInterceptor({
...
//# sourceMappingURL=secure-http-client.service.spec.js.map