...
const _componentsutils = require("../components.utils");
describe('computeSchemaComponents', ()=>{
    ...
    it('should compute schema components', ()=>{
        const flatObjectMetadata = {
            ...
            fieldIds: _objectmetadataitemmock.objectMetadataItemMock.fields.map((f)=>f.id)
        };
        const flatFieldMetadataMaps = {
            byUniversalIdentifier: Object.fromEntries(_objectmetadataitemmock.objectMetadataItemMock.fields.map((f)=>[
                    f.universalIdentifier || f.id,
                    f
                ])),
            universalIdentifierById: Object.fromEntries(_objectmetadataitemmock.objectMetadataItemMock.fields.map((f)=>[
                    f.id,
                    f.universalIdentifier || f.id
                ])),
        ...
        const flatObjectMetadataMaps = {
            byUniversalIdentifier: {
                ...
                [relationTargetObjectMetadata.universalIdentifier]: relationTargetObjectMetadata
            },
            universalIdentifierById: {
                [flatObjectMetadata.id]: flatObjectMetadata.universalIdentifier,
                [relationTargetObjectMetadata.id]: relationTargetObjectMetadata.universalIdentifier
            },
            universalIdentifiersByApplicationId: {}
        };
        expect((0, _componentsutils.computeSchemaComponents)([
            flatObjectMetadata
        ], flatObjectMetadataMaps, flatFieldMetadataMaps)).toMatchInlineSnapshot(`
{
  "ObjectName": {
...
`);
    ...
    it.each(testsCases)('$title', ({ context: field })=>{
        ...
        const flatFieldMetadataMaps = {
            byUniversalIdentifier: {
                [field.universalIdentifier || field.id]: field
            },
            universalIdentifierById: {
        ...
        expect((0, _componentsutils.computeSchemaComponents)([
            flatObjectMetadata
        ], flatObjectMetadataMaps, flatFieldMetadataMaps)).toMatchSnapshot();
    });
});
