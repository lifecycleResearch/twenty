...
const _validatedstoragedriver = require("../validated-storage.driver");
const createMockDriver = ()=>({
        readFile: jest.fn().mockResolvedValue(_stream.Readable.from([])),
...
describe('ValidatedStorageDriver', ()=>{
    ...
    beforeEach(()=>{
        ...
        driver = new _validatedstoragedriver.ValidatedStorageDriver(mockDelegate);
    });
    describe('delegates to the underlying driver for safe paths', ()=>{
        ...
        it('should delegate checkFolderExists', async ()=>{
            await driver.checkFolderExists({
                folderPath: 'folder'
            });
        ...
        it('should delegate getPresignedUrl', async ()=>{
            mockDelegate.getPresignedUrl.mockResolvedValue('https://s3.example.com/signed');
    ...
    describe('rejects path traversal attempts', ()=>{
        ...
        it('should reject downloadFolder with traversal', async ()=>{
            await expect(driver.downloadFolder({
                onStoragePath: '../../../etc',
                localPath: '/tmp/local'
            })).rejects.toMatchObject({
                code: _filestorageexception.FileStorageExceptionCode.ACCESS_DENIED
        ...
        it('should reject getPresignedUrl with traversal', async ()=>{
            await expect(driver.getPresignedUrl({
                filePath: '../../../etc/passwd'
            })).rejects.toMatchObject({
                code: _filestorageexception.FileStorageExceptionCode.ACCESS_DENIED
            });
...
//# sourceMappingURL=validated-storage.driver.spec.js.map