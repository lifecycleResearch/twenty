...
let TestDatabaseConfigDriver = class TestDatabaseConfigDriver extends _databaseconfigdriver.DatabaseConfigDriver {
    get testAllPossibleConfigKeys() {
...
describe('DatabaseConfigDriver', ()=>{
    ...
    let configStorage;
    beforeEach(async ()=>{
        _isenvonlyconfigvarutil.isEnvOnlyConfigVar.mockImplementation((key)=>{
        ...
        const module = await _testing.Test.createTestingModule({
            providers: [
                ...
                {
                    provide: _configcacheservice.ConfigCacheService,
                    useValue: {
                        get: jest.fn(),
                        set: jest.fn(),
                        clear: jest.fn(),
                ...
                {
                    provide: _configstorageservice.ConfigStorageService,
                    useValue: {
                        get: jest.fn(),
                        set: jest.fn(),
                        loadAll: jest.fn()
        ...
        driver = module.get(_databaseconfigdriver.DatabaseConfigDriver);
        configCache = module.get(_configcacheservice.ConfigCacheService);
        configStorage = module.get(_configstorageservice.ConfigStorageService);
        jest.clearAllMocks();
    });
    afterEach(()=>{
        jest.clearAllMocks();
    ...
    describe('refreshAllCache', ()=>{
        ...
        it('should handle errors gracefully and verify cache remains unchanged', async ()=>{
            ...
            jest.spyOn(configCache, 'getAllKeys').mockReturnValue([
                CONFIG_PASSWORD_KEY
            ]);
            jest.spyOn(configCache, 'get').mockImplementation((key)=>mockCacheState.get(key));
            await driver.refreshAllCache();
            expect(driver['logger'].error).toHaveBeenCalled();
            expect(configStorage.loadAll).toHaveBeenCalled();
...
//# sourceMappingURL=database-config.driver.spec.js.map