...
jest.mock('src/engine/core-modules/twenty-config/constants/config-variables-masking-config', ()=>({
...
jest.mock('src/engine/core-modules/twenty-config/utils/is-env-only-config-var.util', ()=>({
...
const setupTestModule = async (isDatabaseConfigEnabled = true)=>{
    ...
    const module = await _testing.Test.createTestingModule({
        providers: [
            ...
            {
                provide: _databaseconfigdriver.DatabaseConfigDriver,
                useValue: {
                    get: jest.fn(),
                    set: jest.fn(),
                    update: jest.fn(),
    ...
    return {
        service: module.get(_twentyconfigservice.TwentyConfigService),
        databaseConfigDriver: module.get(_databaseconfigdriver.DatabaseConfigDriver),
        environmentConfigDriver: module.get(_environmentconfigdriver.EnvironmentConfigDriver),
        configService: module.get(_config.ConfigService),
        configVariablesInstance: module.get(_configvariablesinstancetokensconstants.CONFIG_VARIABLES_INSTANCE_TOKEN)
    };
};
...
const setPrivateProps = (service, props)=>{
...
describe('TwentyConfigService', ()=>{
    let service;
    let databaseConfigDriver;
    let environmentConfigDriver;
    beforeEach(async ()=>{
        ...
        _typedreflect.TypedReflect.getMetadata.mockReturnValue(mockConfigVarMetadata);
    });
    afterEach(()=>{
        jest.clearAllMocks();
    });
    ...
    describe('constructor', ()=>{
        it('should set isDatabaseDriverActive to false when database config is disabled', async ()=>{
            const { service, environmentConfigDriver } = await setupTestModuleWithoutDb();
            expect(environmentConfigDriver.get).toHaveBeenCalledWith('IS_CONFIG_VARIABLES_IN_DB_ENABLED');
    ...
    describe('get', ()=>{
        ...
        beforeEach(()=>{
            _isenvonlyconfigvarutil.isEnvOnlyConfigVar.mockReturnValue(false);
        });
        it('should use environment driver for environment-only variables', ()=>{
            _isenvonlyconfigvarutil.isEnvOnlyConfigVar.mockReturnValue(true);
    ...
    describe('update', ()=>{
        ...
        it('should throw error when database driver is not active', async ()=>{
            ...
            await expect(service.update('TEST_VAR', 'new value')).rejects.toThrow('Database configuration is disabled or unavailable, cannot update configuration');
        });
        it('should throw error when updating environment-only variable', async ()=>{
            setPrivateProps(service, {
                isDatabaseDriverActive: true
            });
            _typedreflect.TypedReflect.getMetadata.mockReturnValue({
                ENV_ONLY_VAR: {
                    isEnvOnly: true
                }
            });
            await expect(service.update('ENV_ONLY_VAR', 'new value')).rejects.toThrow('Cannot update environment-only variable: ENV_ONLY_VAR');
        });
        ...
        it('should propagate errors from database driver', async ()=>{
            ...
            await expect(service.update('TEST_VAR', 'new value')).rejects.toThrow(error);
        });
    });
    describe('getMetadata', ()=>{
        it('should return metadata for a config variable', ()=>{
            const result = service.getMetadata('TEST_VAR');
            expect(result).toEqual(mockConfigVarMetadata.TEST_VAR);
        });
        it('should return undefined when metadata does not exist', ()=>{
            const result = service.getMetadata('UNKNOWN_VAR');
            expect(result).toBeUndefined();
        });
    });
    describe('getAll', ()=>{
        const setupDriverMocks = ()=>{
            jest.spyOn(environmentConfigDriver, 'get').mockImplementation((key)=>{
            ...
            jest.spyOn(databaseConfigDriver, 'get').mockImplementation((key)=>{
                const keyStr = String(key);
                // @ts-expect-error legacy noImplicitAny
        ...
        it('should return all config variables with environment source when database driver is not active', ()=>{
            ...
            expect(result).toEqual({
                TEST_VAR: {
                    ...
                    source: _configsourceenum.ConfigSource.ENVIRONMENT
                },
                ENV_ONLY_VAR: {
        ...
        it('should return config variables with database source when database driver is active', ()=>{
            ...
            expect(result.TEST_VAR).toEqual({
                value: 'db test value',
                metadata: mockConfigVarMetadata.TEST_VAR,
                source: _configsourceenum.ConfigSource.DATABASE
            });
    ...
    describe('getCacheInfo', ()=>{
        ...
        it('should return cache stats when database driver is active', ()=>{
            ...
            jest.spyOn(databaseConfigDriver, 'getCacheInfo').mockReturnValue(cacheStats);
            const result = service.getCacheInfo();
            expect(result).toEqual({
    ...
    describe('validateConfigVariableExists', ()=>{
        ...
        it('should be called by set, update, and delete methods', async ()=>{
            const validateSpy = jest.spyOn(service, 'validateConfigVariableExists').mockReturnValue(true);
            setPrivateProps(service, {
                isDatabaseDriverActive: true
            });
            jest.spyOn(service, 'validateNotEnvOnly').mockImplementation(()=>{});
            await service.set('TEST_VAR', 'test value');
            await service.update('TEST_VAR', 'updated value');
            await service.delete('TEST_VAR');
        ...
        it('should return true for valid config variables with metadata', ()=>{
            ...
            jest.spyOn(service, 'getMetadata').mockReturnValue(mockConfigVarMetadata.TEST_VAR);
        ...
        it('should throw error when config variable does not exist', ()=>{
            ...
            }).toThrow('Config variable "MISSING_KEY" does not exist in ConfigVariables');
        });
    });
});

//# sourceMappingURL=twenty-config.service.spec.js.map