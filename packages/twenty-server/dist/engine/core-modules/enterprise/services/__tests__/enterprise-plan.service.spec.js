...
jest.mock('crypto', ()=>({
        ...
        verify: (...args)=>mockCryptoVerify(...args)
    }));
...
describe('EnterprisePlanService', ()=>{
    ...
    let originalFetch;
    const setupEnterpriseKey = (key)=>{
        configGetMock.mockImplementation((configKey)=>{
            if (configKey === 'ENTERPRISE_KEY') return key;
            if (configKey === 'ENTERPRISE_API_URL') return MOCK_API_URL;
            return undefined;
        });
    };
    const setupValidState = async (overrides = {})=>{
        const { keyPayload = MOCK_KEY_PAYLOAD, validityPayload = MOCK_VALIDITY_PAYLOAD, cryptoVerifyResult = true } = overrides;
    ...
    beforeEach(async ()=>{
            ...
            providers: [
                ...
                },
                {
                    ...
                    useValue: {
                        ...
                        target: _apptokenentity.AppTokenEntity,
                        manager: {
        ...
        service = module.get(_enterpriseplanservice.EnterprisePlanService);
    });
    afterEach(()=>{
    ...
    describe('isValidEnterpriseKeyFormat', ()=>{
        ...
        it('should accept production key when NODE_ENV is development', ()=>{
            configGetMock.mockImplementation((key)=>{
                if (key === 'NODE_ENV') return _nodeenvironmentinterface.NodeEnvironment.DEVELOPMENT;
                if (key === 'ENTERPRISE_API_URL') return MOCK_API_URL;
                return undefined;
            });
    ...
    describe('setEnterpriseKey', ()=>{
        it('should set the enterprise key via config service', async ()=>{
            ...
            expect(configSetMock).toHaveBeenCalledWith('ENTERPRISE_KEY', 'new-enterprise-key');
        });
        it('should throw specific error when DB config is disabled', async ()=>{
            configSetMock.mockRejectedValue(new _twentyconfigexception.ConfigVariableException('Database config disabled', _twentyconfigexception.ConfigVariableExceptionCode.DATABASE_CONFIG_DISABLED));
            await expect(service.setEnterpriseKey('key')).rejects.toThrow('IS_CONFIG_VARIABLES_IN_DB_ENABLED is false on your server');
        });
        it('should re-throw other errors', async ()=>{
            configSetMock.mockRejectedValue(new Error('Unexpected error'));
            await expect(service.setEnterpriseKey('key')).rejects.toThrow('Unexpected error');
        });
    });
        ...
        it('should refresh and return true when API call succeeds', async ()=>{
            ...
            fetchMock.mockResolvedValue({
                ...
                json: ()=>Promise.resolve({
                        validityToken: fakeValidityToken
                    })
            });
    ...
    describe('reportSeats', ()=>{
        ...
        it('should report seats and return true on success', async ()=>{
            ...
            expect(result).toBe(true);
            expect(fetchMock).toHaveBeenCalledWith(`${MOCK_API_URL}/seats`, expect.objectContaining({
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                }
    ...
    describe('getPortalUrl', ()=>{
        ...
        it('should return portal URL on success', async ()=>{
            ...
            expect(fetchMock).toHaveBeenCalledWith(`${MOCK_API_URL}/portal`, expect.objectContaining({
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                }
...
//# sourceMappingURL=enterprise-plan.service.spec.js.map