...
const _usevalidategraphqlquerycomplexityhook = require("../use-validate-graphql-query-complexity.hook");
describe('useValidateGraphqlQueryComplexity', ()=>{
    const validateQuery = (query, options)=>{
        const plugin = (0, _usevalidategraphqlquerycomplexityhook.useValidateGraphqlQueryComplexity)(options);
        if (!(0, _utils.isDefined)(plugin.onParse)) {
    ...
    describe('maximumAllowedRootResolvers', ()=>{
      ...
      `;
            ...
            expect(error).not.toBeNull();
            expect(error?.message).toContain('Query too complex - Too many root resolvers requested: 6 - Maximum allowed root resolvers: 4');
        });
    ...
    describe('maximumAllowedNestedFields', ()=>{
      ...
      `;
            const error = validateQuery(query, {
                maximumAllowedNestedFields: 1
            });
            expect(error).not.toBeNull();
            expect(error?.message).toContain('Query too complex - Too many nested fields requested: 6 - Maximum allowed nested fields: 1');
        });
    });
...
//# sourceMappingURL=use-validate-graphql-query-complexity.hook.spec.js.map