...
const _promisememoizerstorage = require("../promise-memoizer.storage");
describe('PromiseMemoizer', ()=>{
    ...
    beforeEach(()=>{
        ...
        memoizer = new _promisememoizerstorage.PromiseMemoizer(TTL_MS);
    });
    describe('memoizePromiseAndExecute', ()=>{
        it('should execute factory and cache result', async ()=>{
            ...
            const result = await memoizer.memoizePromiseAndExecute('test-key-1', mockFactory);
        ...
        it('should return cached value within TTL', async ()=>{
            ...
            jest.spyOn(global.Date, 'now').mockImplementation(()=>currentTime + TTL_MS / 2);
            const result = await memoizer.memoizePromiseAndExecute('test-key-1', mockFactory);
            expect(result).toBe('test-value');
            expect(mockFactory).toHaveBeenCalledTimes(1);
        });
        it('should re-execute factory after TTL expires', async ()=>{
            ...
            const currentTime = Date.now();
            jest.spyOn(global.Date, 'now').mockImplementation(()=>currentTime + TTL_MS + 100);
            const result = await memoizer.memoizePromiseAndExecute('test-key-1', mockFactory);
            expect(result).toBe('test-value');
        ...
        it('should deduplicate concurrent requests', async ()=>{
            ...
            const promise1 = memoizer.memoizePromiseAndExecute('test-key-1', mockFactory);
            const promise2 = memoizer.memoizePromiseAndExecute('test-key-1', mockFactory);
...
//# sourceMappingURL=promise-memoizer.storage.spec.js.map