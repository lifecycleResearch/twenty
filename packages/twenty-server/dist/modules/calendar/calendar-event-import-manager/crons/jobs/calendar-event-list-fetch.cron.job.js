    ...
    async handle() {
        ...
        for (const activeWorkspace of activeWorkspaces){
            try {
                ...
                const [calendarChannels] = isMigrated ? await this.coreDataSource.query(`UPDATE core."calendarChannel" SET "syncStage" = '${_calendarchannelworkspaceentity.CalendarChannelSyncStage.CALENDAR_EVENT_LIST_FETCH_SCHEDULED}', "syncStageStartedAt" = COALESCE("syncStageStartedAt", '${now}')
               WHERE "workspaceId" = '${activeWorkspace.id}' AND "isSyncEnabled" = true AND "syncStage" = '${_calendarchannelworkspaceentity.CalendarChannelSyncStage.CALENDAR_EVENT_LIST_FETCH_PENDING}' RETURNING *`) : await this.coreDataSource.query(`UPDATE ${(0, _getworkspaceschemanameutil.getWorkspaceSchemaName)(activeWorkspace.id)}."calendarChannel" SET "syncStage" = '${_calendarchannelworkspaceentity.CalendarChannelSyncStage.CALENDAR_EVENT_LIST_FETCH_SCHEDULED}', "syncStageStartedAt" = COALESCE("syncStageStartedAt", '${now}')
               WHERE "isSyncEnabled" = true AND "syncStage" = '${_calendarchannelworkspaceentity.CalendarChannelSyncStage.CALENDAR_EVENT_LIST_FETCH_PENDING}' RETURNING *`);
