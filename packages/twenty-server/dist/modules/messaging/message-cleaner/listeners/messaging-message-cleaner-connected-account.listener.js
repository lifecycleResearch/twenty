...
let MessagingMessageCleanerConnectedAccountListener = class MessagingMessageCleanerConnectedAccountListener {
    async handleDestroyedEvent(payload) {
        await Promise.all(payload.events.map((eventPayload)=>this.messageQueueService.add(_messagingconnectedaccountdeletioncleanupjob.MessagingConnectedAccountDeletionCleanupJob.name, {
...
], MessagingMessageCleanerConnectedAccountListener.prototype, "handleDestroyedEvent", null);
