...
describe('Gmail MIME Message Format', ()=>{
    it('should create valid multipart/alternative MIME structure', ()=>{
        ...
        const headers = [];
        headers.push(`From: "${(0, _mimeencodeutil.mimeEncode)(fromName)}" <${fromEmail}>`);
        headers.push(`To: ${sendMessageInput.to}`, `Subject: ${(0, _mimeencodeutil.mimeEncode)(sendMessageInput.subject)}`, 'MIME-Version: 1.0', `Content-Type: multipart/alternative; boundary="${boundary}"`, '', `--${boundary}`, 'Content-Type: text/plain; charset="UTF-8"', '', sendMessageInput.body, '', `--${boundary}`, 'Content-Type: text/html; charset="UTF-8"', '', sendMessageInput.html, '', `--${boundary}--`);
...
//# sourceMappingURL=gmail-mime-message.spec.js.map