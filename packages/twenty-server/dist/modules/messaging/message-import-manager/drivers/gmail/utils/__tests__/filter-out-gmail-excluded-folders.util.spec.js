...
const _filtergmailmessagesbyfolderpolicyutil = require("../filter-gmail-messages-by-folder-policy.util");
const createMessage = (externalId, labelIds)=>({
        externalId,
        labelIds
    });
const createFolder = (externalId, isSynced)=>({
        externalId,
        isSynced
    });
describe('filterGmailMessagesByFolderPolicy', ()=>{
    describe('ALL_FOLDERS policy', ()=>{
        it('bypasses all filtering', ()=>{
            ...
            const result = (0, _filtergmailmessagesbyfolderpolicyutil.filterGmailMessagesByFolderPolicy)(messages, {
                messageFolders: [
    ...
    describe('only custom labels synced', ()=>{
        ...
        it('includes message with synced label even if it also has non-synced labels', ()=>{
            ...
            expect(result.map((m)=>m.externalId)).toEqual([
                '1'
            ]);
        });
...
//# sourceMappingURL=filter-out-gmail-excluded-folders.util.spec.js.map