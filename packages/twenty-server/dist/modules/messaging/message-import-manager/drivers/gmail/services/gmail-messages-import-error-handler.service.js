...
Object.defineProperty(exports, "GmailMessagesImportErrorHandler", {
    enumerable: true,
    get: function() {
        return GmailMessagesImportErrorHandler;
    }
});
...
let GmailMessagesImportErrorHandler = class GmailMessagesImportErrorHandler {
    handleError(error, messageExternalId) {
        this.logger.error(`Gmail: Error importing message ${messageExternalId}: ${JSON.stringify(error)}`);
        ...
        throw new _messageimportdriverexception.MessageImportDriverException(`Gmail message import error: ${error instanceof Error ? error.message : String(error)}`, _messageimportdriverexception.MessageImportDriverExceptionCode.UNKNOWN);
    }
    constructor(){
        this.logger = new _common.Logger(GmailMessagesImportErrorHandler.name);
    }
};
GmailMessagesImportErrorHandler = _ts_decorate([
    (0, _common.Injectable)(),
    _ts_metadata("design:type", Function),
    _ts_metadata("design:paramtypes", [])
], GmailMessagesImportErrorHandler);

//# sourceMappingURL=gmail-messages-import-error-handler.service.js.map