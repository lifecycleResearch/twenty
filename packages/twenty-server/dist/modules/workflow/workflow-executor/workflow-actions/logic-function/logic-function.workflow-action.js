...
Object.defineProperty(exports, "LogicFunctionWorkflowAction", {
    enumerable: true,
    get: function() {
        return LogicFunctionWorkflowAction;
    }
});
...
const _isworkflowlogicfunctionactionguard = require("./guards/is-workflow-logic-function-action.guard");
...
let LogicFunctionWorkflowAction = class LogicFunctionWorkflowAction {
    async execute({ currentStepId, steps, context, runInfo }) {
        const step = (0, _findsteporthrowutil.findStepOrThrow)({
            stepId: currentStepId,
            steps
        });
        if (!(0, _isworkflowlogicfunctionactionguard.isWorkflowLogicFunctionAction)(step)) {
            throw new _workflowstepexecutorexception.WorkflowStepExecutorException('Step is not a logic function action', _workflowstepexecutorexception.WorkflowStepExecutorExceptionCode.INVALID_STEP_TYPE);
        }
        ...
        const logicFunction = (0, _findflatentitybyidinflatentitymapsutil.findFlatEntityByIdInFlatEntityMaps)({
            flatEntityId: workflowActionInput.logicFunctionId,
            flatEntityMaps: flatLogicFunctionMaps
        });
        if (!logicFunction) {
            throw new _workflowstepexecutorexception.WorkflowStepExecutorException(`Logic function with id ${workflowActionInput.logicFunctionId} not found`, _workflowstepexecutorexception.WorkflowStepExecutorExceptionCode.INVALID_STEP_TYPE);
        }
    ...
    constructor(logicFunctionExecutorService, flatEntityMapsCacheService){
        this.logicFunctionExecutorService = logicFunctionExecutorService;
        this.flatEntityMapsCacheService = flatEntityMapsCacheService;
    }
};
LogicFunctionWorkflowAction = _ts_decorate([
...
//# sourceMappingURL=logic-function.workflow-action.js.map